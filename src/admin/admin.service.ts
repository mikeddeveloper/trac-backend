...
export class AdminService {
  ...
  async sendCommunityUpdate(requesterId: string) {
    const requester = await this.userRepo.findOne({ where: { id: requesterId } });
    if (await this.resolveAdminRole(requester) !== 'super_admin') {
      throw new ForbiddenException('Only a super administrator can send platform announcements');
    }
    ...
    const failed: string[] = [];
    for (const user of users) {
      const campaignKey = `campaign:community-update-2026-09:${user.id}`;
      const existing = await this.userRepo.query('SELECT 1 FROM platform_settings WHERE key = $1 LIMIT 1', [campaignKey]);
      if (existing.length) { skipped++; continue; }
      const result = await this.emailService.sendCommunityUpdateEmail({ fullName: user.fullName, email: user.email, role: user.role });
      if (!result.success) { failed.push(user.email); continue; }
      await this.userRepo.query(
        'INSERT INTO platform_settings (key, value, updated_at) VALUES ($1, $2::jsonb, now()) ON CONFLICT (key) DO NOTHING',
        [campaignKey, JSON.stringify({ sentAt: new Date().toISOString(), email: user.email })],
    ...
    return { message: 'Community update campaign completed', recipients: users.length, sent, skipped, failed: failed.length, failedEmails: failed };
  }
  ...
  async releasePayment(paymentId: string) {
    ...
    if (payment.status === PaymentStatus.RELEASED) return { message: 'Withdrawal has already been approved', status: 'released' };
    if (payment.status !== PaymentStatus.HELD) throw new BadRequestException('The transporter must request withdrawal before admin approval');
    ...
    if (!job.transporterId) throw new BadRequestException('This delivery has no assigned transporter');
    const result = await this.paymentsService.approveWithdrawal(job.id, job.transporterId);
