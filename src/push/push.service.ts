      ...
      for (const sub of subs) {
        try {
          if(sub.endpoint.startsWith('ExponentPushToken[')||sub.endpoint.startsWith('ExpoPushToken[')){
            const expoResponse=await fetch('https://exp.host/--/api/v2/push/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:sub.endpoint,title:payload.title,body:payload.body,data:{url:payload.url,...payload.data},sound:'default',channelId:'delivery-updates-v2'})});
